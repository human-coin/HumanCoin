...
unsigned int nTargetSpacing = 1 * 70; // 60 seconds
unsigned int nStakeMinAge = 24 * 60 * 60; // 24 hour
unsigned int nStakeMaxAge = -1;           //unlimited
unsigned int nModifierInterval = 10 * 70; // time to elapse before new modifier is computed
...
const string strMessageMagic = "HumanCoin Signed Message:\n";
...
const int DAILY_BLOCKCOUNT =  1234;
const int YEARLY_BLOCKCOUNT =  450410;
...
static const int64_t nTargetTimespan = 1 * 70;  // every block
//
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "HumanCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Human Coin";
        CTransaction txNew;
        txNew.nTime = 1400330150;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x26cf80f818499a690d410583f6d86e27eed564569adf52cc1b2faa3543f2222f"));
        block.print();
